...
at least nr_verts vertices. Returned faces are zero-indexed!urns a geodesic icosahedron with subdivision frequency nu.

Original python code by vand@dtu.dk, 2014, 2017, 2021.
Originally developed in connectiton with 
https://ieeexplore.ieee.org/document/7182720
 
"""
function icosphere(nu = 1, r = 1, nr_verts = nothing)
...
function icosahedron(r)
    ...
    vertices = Vector{SVector{3, Float32}}([[0, 1, ϕ], [0, -1, ϕ], [1, ϕ, 0], [-1, ϕ, 0], [ϕ, 0, 1], [-ϕ, 0, 1]]*(r/sqrt(1 + ϕ^2)))
    vertices = vcat(vertices, -vertices)

    faces = Vector{SVector{3, Int}}([[1, 6, 2], [1, 4, 6], [1, 3, 4], [1, 5, 3], [1, 2, 5],
...
function subdivide_mesh(vertices, faces, nu)
    ...
    for i in range(1, length(faces))
       append!(edges, [sort([faces[i][1], faces[i][2]]), sort([faces[i][2], faces[i][3]]), sort([faces[i][1], faces[i][3]])])
    end
    ...
    #total number of (all) vertices === subvertices would be the number of counted vertices, plus
    #the number of (sub-)vertices on each edge, plus the number of vertices on the inner faces
    subvertices = Vector{SVector{3, Float32}}(undef, Int(V + E*(nu - 1) + F*floor((nu - 1)*(nu - 2)/2)))
    ...
    reordered_template = Vector{Int32}[]
    ...
    for e in range(1, E) #e is the number of edge, (e - 1) would count the number of counted edges
        edge = edges[e]
        
        for k in range(1, nu - 1)  #k gives the number of subvertices on the edge
            subvertices[V + (e - 1)*(nu - 1) + k] = (w[end - k + 1]*vertices[edge[1]] + w[k]*vertices[edge[2]])
        end
    end

    r = collect(range(1, nu - 1))
    for f in range(1, F)
        T = collect(range(Int(floor((f - 1)*(nu - 1)*(nu - 2)/2)) + E*(nu - 1) + V + 1, Int(floor((f)*(nu - 1)*(nu - 2)/2)) + E*(nu - 1) + V))
        ...
        for i in 1:nu^2
            subfaces[(f - 1)*nu^2 + i] = [VEF[reordered_template[i][1]], VEF[reordered_template[i][2]], VEF[reordered_template[i][3]]]
        end
...
function faces_template(nu)
    ...
    for i in range(1, nu)
        ...
        for j in range(1, i - 1)
            append!(faces, [[j + vertex, j + vertex + skip, j + vertex + skip + 1]])
            append!(faces, [[j + vertex, j + vertex + skip + 1, j + vertex + 1]])
        end
        #the last face that is unpaired
        append!(faces, [[i + vertex, i + vertex + skip, i + vertex + skip + 1]])
    end
...
function vertex_ordering(nu)
    ...
    inside = [j for j in range(3*nu + 1, Int(floor((nu + 1)*(nu + 2)/2)))]

    o = [1] #topmost corner
    for i in range(1, nu - 1)
        append!(o, left[i])
        append!(o, inside[Int(floor((i - 1)*(i - 2)/2)) + 1 : Int(floor((i - 1)*i/2))])
        append!(o, right[i])
    end
    append!(o, 2)
    append!(o, bottom)
...
function inside_points(vAB, vAC)
    ...
    if l == 1
        ...
        for i in range(1, length(vAB) - 1)
            ...
            for k in range(1, i)
                append!(v, [SVector(w[end - k + 1].*vAB[i + 1][:] + w[k].*vAC[i + 1][:])])
            end
        end
    end
